...
/**
 * Main class that controls the flow of our pathfinding program. Handles user
 * interactions and coordinates between UI, Map, and RoutePlanner.
 */
public class Main {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			col = Math.min((int) ((Main.y - 40) / 75), 9);
			row = Math.min((int) ((Main.x - 20) / 75), 9);
			...
			if (state == 1) {
				...
				if (!(Map.grid[endNode.getRow()][endNode.getCol()]
						.equals(Map.grid[startNode.getRow()][startNode.getCol()])))
				...
				if (!(Map.grid[endNode.getRow()][endNode.getCol()]
						.equals(Map.grid[obstacleNode.getRow()][obstacleNode.getCol()]))
						&& !(Map.grid[startNode.getRow()][startNode.getCol()]
								.equals(Map.grid[obstacleNode.getRow()][obstacleNode.getCol()])))
